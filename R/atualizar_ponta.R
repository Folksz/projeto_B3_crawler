...
#' 
#' 
atualizar_ponta<-function(diretorio="output/",gerar_cotacao=TRUE){
  print(getwd())
  ...
  if(gerar_cotacao==TRUE){
    ...
    tbl_cotacoes=rbind(tbl_cotacoes%>%mutate(dte_data=as.Date(dte_data)),anti_join(tbl_cotacoes_ponta,tbl_cotacoes%>%mutate(dte_data=as.Date(dte_data))))
    utils::write.csv(tbl_cotacoes,paste(diretorio,"tbl_cotacoes.csv",sep=""),row.names=FALSE)
  }
