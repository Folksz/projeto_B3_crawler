...
server <- function(input, output) {
  ...
  output$cotacao_entre_empresa_valor <- renderPlotly({
    montar_cotacao_entre_empresa(
      tbl_cotacao = dfs$tbl_cotacoes,
  ...
  output$multiplo_entre_empresa_linha <- renderPlotly({
    montar_multiplo_entre_empresas(
      tbl_dre = dfs$tbl_dre,
      tbl_bp = rbind(dfs$tbl_bp_ativo, dfs$tbl_bp_passivo),
  ...
  output$variaveis_macro <-
    renderPlotly({
      montar_variavel_macro(input$input_variavel_macro)
    })
  
  output$divulgacao_entre_empresa <-
    renderPlotly({
      divulgacao_entre_empresa(
        tbl_divulgacoes = dfs$tbl_divulgacoes,
        tbl_empresas = dfs$tbl_empresas,
        empresas_selec =    input$input_empresas_divulgacao
      )
    })
...
criar_painel <- function(diretorio = "") {
  dfs <<- importar_bd(diretorio)
  ui <- shinydashboard::dashboardPage(construir_header(),
                                      construir_sidebar(),
                                      construir_body())
