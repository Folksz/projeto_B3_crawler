montar_DRE_por_empresa<-function(tbl_dre=dfs$tbl_dre,
  ...
  if(anual_trimestral=="anual"){
    tbl_dre_empresa=tbl_dre_empresa%>%
      ...
      summarise(dte_data=last(lubridate::year(dte_data)),ds_conta=last(ds_conta),
...
montar_BP_por_empresa<-function(tbl_bp=rbind(dfs$tbl_bp_ativo,dfs$tbl_bp_passivo),
                                tbl_ds_conta,
...
montar_DRE_entre_empresa<-function(tbl_dre=dfs$tbl_dre,
  ...
  if(anual_trimestral=="anual"){
    tbl_dre_empresa=tbl_dre_empresa%>%
      ...
      summarise(dte_data=last(lubridate::year(dte_data)),
                regex=last(regex),
                vl_conta=sum(vl_conta,na.rm=TRUE))%>%
      ungroup()
  }
...
montar_multiplo_entre_empresas<-function(
  ...
  return(plot_ly(tbl_painel_metricas%>%arrange(dte_data),
                 x=~dte_data,
                 y=~qual_variavel,
                 color=~str_empresa,
                 type=type,
