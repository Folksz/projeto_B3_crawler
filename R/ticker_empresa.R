...
construir_depara <- function(bulk) {
  tbl_CVM = data.frame(unique(
    bulk$itr$itr_cia_aberta_ %>% select(str_cia_CVM = DENOM_CIA, CNPJ_CIA)
  ))
  ...
  tbl_ticker_empresa = tbl_ticker_empresa %>% mutate(
    regex = stringr::str_replace_all(str_razao_social_semacento, "[^[:alnum:]]", ""),
    regex = stringr::str_replace_all(regex, "BANCO", "BCO")
  )
  tbl_CVM = tbl_CVM %>% mutate(
    regex = stringr::str_replace_all(str_cia_CVM_semacento, "[^[:alnum:]]", ""),
    regex = stringr::str_replace_all(regex, "BANCO", "BCO")
  )
  ...
  tbl_empresas = merge(
    tbl_empresas %>% mutate(
      regex = stringr::str_replace_all(str_ticker, "[^[:alnum:]]", ""),
      regex = gsub('[0-9]+', '', regex)
    ),
    tbl_setores %>% mutate(regex = str_codigo),
    by = "regex"
  )
