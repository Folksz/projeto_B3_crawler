construir_body <- function() {
  dre_por_empresa = tabItem(tabName = "dre_por_empresa",
                            fluidRow(
                              ...
                              box(
                                selectInput(
                                  "input_empresa_unique_dre",
                                  "Empresa",
                                  choices = dfs$tbl_empresas$regex,
                                  selected = "MGLU"
                                )
                              ),
                              box(
                                selectInput(
                                  "input_contas_multiple_dre",
                                  "Contas",
                                  choices = unique((
                                    dfs$tbl_ds_conta %>% filter(nchar <= 4) %>% filter(str_tabela == "tbl_dre")
                                  )$ds_conta),
                                  multiple = TRUE,
                                  selected = TRUE
                                )
  ...
  bp_por_empresa = tabItem(tabName = "bp_por_empresa",
                           fluidRow(
                             ...
                             box(
                               selectInput(
                                 "input_contas_multiple_bp",
                                 "Contas",
                                 choices = unique((
                                   dfs$tbl_ds_conta %>% filter(nchar <= 4) %>%
                                     filter(str_tabela == "tbl_bp_ativo" |
  ...
  cotacao_entre_empresa = tabItem(tabName = "cotacao_entre_empresa",
                                  fluidRow(box(
                                    selectInput(
                                      "input_empresa_multiple_multiplo",
                                      "Empresa",
                                  ...
                                  fluidRow(
                                    ...
                                    tabBox(
                                      title = "Analise entre Empresa",
                                      id = "tabset4",
                                      height = "250px",
                                      tabPanel("Cotacao", plotlyOutput("cotacao_entre_empresa_cotacao")),
                                      tabPanel(
                                        "Valor de Empresa",
                                        plotlyOutput("cotacao_entre_empresa_valor")
                                      )
                                    )
                                  ))
  ...
  divulgacao_entre_empresa = tabItem(tabName = "divulgacao_entre_empresa",
                                     fluidRow(
                                       tabBox(
                                         title = "BP entre Empresa",
                                         id = "tabset4",
                                         height = "250px",
                                         tabPanel("Linha", plotlyOutput("divulgacao_entre_empresa"))
                                       ),
                                       box(
                                         selectInput(
                                           "input_empresas_divulgacao",
                                           "Empresas",
                                           choices = unique(dfs$tbl_empresas$regex),
                                           selected = TRUE,
                                           multiple = TRUE
