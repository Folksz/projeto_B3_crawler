...
construir_metricas<-function(tbl_dre,tbl_bp,empresas_selec){
  ...
  for(empresa in empresas_selec){
    ...
    tbl_painel_empresa=merge(tbl_dre_empresa%>%reshape2::dcast(dte_data~cd_conta,value.var="vl_conta"),
                             tbl_bp_empresa%>%reshape2::dcast(dte_data~cd_conta,value.var="vl_conta"))
