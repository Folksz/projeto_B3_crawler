...
construir_DRE<-function(bulk){
  ...
  dre=dre%>%filter(ORDEM_EXERC=='ÚLTIMO')%>%filter(lubridate::month(DT_INI_EXERC)==lubridate::month(DT_FIM_EXERC)-2)
  ...
  dre_3tri=dre%>%mutate(ano=lubridate::year(DT_FIM_EXERC))%>%group_by(ano,CD_CONTA,DENOM_CIA)%>%summarise(tres_tri=sum(VL_CONTA,na.rm=TRUE))
  dre_anual=merge(dre_anual%>%mutate(ano=lubridate::year(DT_REFER)),dre_3tri,by=c("ano","CD_CONTA","DENOM_CIA"))
