...
construir_bd<-function(anos,diretorio="output/",gerar_cotacao=FALSE){
  ...
  tbl_divulgacoes=tbl_divulgacoes%>%filter(VERSAO==1)%>%summarise(str_cia_CVM=DENOM_CIA,
                                                    dte_refer=DT_REFER,
                                                    dt_publi=DT_RECEB)
  ...
  tbl_bp_passivo=merge(tbl_bp_passivo,tbl_empresas%>%summarise(str_cia_CVM,regex))
  ...
  tbl_ds_conta=rbind(tbl_dre%>%group_by(paste(cd_conta,ds_conta))%>%
         summarise(ds_conta=last(ds_conta),cd_conta=last(cd_conta),t=n(),str_tabela="tbl_dre")%>%
         ungroup()%>%
         group_by(cd_conta)%>%
         filter(t==max(t)),
         tbl_bp_ativo%>%group_by(paste(cd_conta,ds_conta))%>%
           summarise(ds_conta=last(ds_conta),
                     cd_conta=last(cd_conta),
                     t=n(),str_tabela="tbl_bp_ativo")%>%
           ungroup()%>%
           group_by(cd_conta)%>%
           filter(t==max(t)))
  tbl_ds_conta=rbind(tbl_ds_conta,
                     tbl_bp_passivo%>%group_by(paste(cd_conta,ds_conta))%>%
                       summarise(ds_conta=last(ds_conta),cd_conta=last(cd_conta),t=n(),str_tabela="tbl_bp_passivo")%>%
                       ungroup()%>%
                       group_by(cd_conta)%>%
                       filter(t==max(t)))
  ...
  utils::write.csv(tbl_bp_ativo,paste(diretorio,"tbl_bp_ativo.csv",sep=""),row.names=FALSE)
  utils::write.csv(tbl_bp_passivo,paste(diretorio,"tbl_bp_passivo.csv",sep=""),row.names=FALSE)
  utils::write.csv(tbl_empresas,paste(diretorio,"tbl_empresas.csv",sep=""),row.names=FALSE)
  utils::write.csv(tbl_ticker,paste(diretorio,"tbl_ticker.csv",sep=""),row.names=FALSE)
  utils::write.csv(tbl_divulgacoes,paste(diretorio,"tbl_divulgacoes.csv",sep=""),row.names=FALSE)
  utils::write.csv(tbl_ds_conta,paste(diretorio,"tbl_ds_conta.csv",sep=""),row.names=FALSE)
  if(gerar_cotacao==FALSE){
    tbl_cotacoes=data.frame()
  }
  return(list(tbl_dre=tbl_dre,
